...
class AccountDashboard < Administrate::BaseDashboard
  ...
  enterprise_attribute_types = if WorqChatApp.enterprise?
                                 attributes = {
                                   limits: AccountLimitsField
                                 }

                                 # Only show manually managed features in Chatwoot Cloud deployment
                                 attributes[:manually_managed_features] = ManuallyManagedFeaturesField if WorqChatApp.worqchat_cloud?
  ...
  enterprise_show_page_attributes = if WorqChatApp.enterprise?
                                      attrs = %i[custom_attributes limits]
                                      attrs << :manually_managed_features if WorqChatApp.worqchat_cloud?
                                      attrs << :all_features
                                      attrs
                                    else
                                      []
  ...
  enterprise_form_attributes = if WorqChatApp.enterprise?
                                 attrs = %i[limits]
                                 attrs << :manually_managed_features if WorqChatApp.worqchat_cloud?
                                 attrs << :all_features
                                 attrs
